...
`,d=document.createElement("template");d.innerHTML=`
  ...
  <div class="heart">
    ...
    <div class="cup">
      ...
      `}}const T="miles-zoom";customElements.get(T)||customElements.define(T,L);class q extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.innerHTML=`
          <style>
            ${s}
        ...
        `}}const H="miles-pod";customElements.get(H)||customElements.define(H,q);const Re=`:host{display:inline-block;width:inherit;box-sizing:border-box}
...
`;class z extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.appendChild($.content.cloneNode(!0)),this.svg=e.querySelector("svg")}static get observedAttributes(){return["width","height","color"]}attributeChangedCallback(e,i,t){e==="width"&&this.svg.setAttribute("width",t),e==="height"&&this.svg.setAttribute("height",t),e==="color"&&this.svg.setAttribute("fill",t)}}const N="miles-logo";customElements.get(N)||customElements.define(N,z);const R=n=>[null,"false"].includes(n)===!1,P=document.createElement("template");P.innerHTML=`
        <style>
          ${s}
      ...
      `;class B extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.append(P.content.cloneNode(!0)),this.buttontarget=e.querySelector("#buttonTarget")}static get observedAttributes(){return["href","color","selected"]}attributeChangedCallback(e,i,t){e==="href"&&this.buttontarget.setAttribute("href",t),e==="color"&&this.style.setProperty("--color",t),e==="selected"&&(this.buttontarget.setAttribute("aria-selected",R(t)),this.buttontarget.classList.toggle("selected",R(t)))}connectedCallback(){}}const j="miles-button-anchor";customElements.get(j)||customElements.define(j,B);const Pe=`:host{display:inline-block;--width: 215px}.profile-card{display:flex;flex-direction:column;width:var(--width);border-radius:5px;overflow:hidden}figure{border-radius:.5rem;height:var(--width);margin:0 0 .5rem;padding:0}img{background-color:#fff;filter:grayscale(1);width:100%;object-fit:cover;aspect-ratio:1 / 1;animation:fadeIn 1s cubic-bezier(.46,.03,.05,.99)}@keyframes fadeIn{0%{opacity:0}to{opacity:1}}.profile-card__info{display:flex;background-color:var(--miles_secondary_four);padding:1.2rem;min-height:140px;flex-direction:column}.jobtitle{flex-grow:1}.jobtitle p{overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:2;line-clamp:2;-webkit-box-orient:vertical}.description{display:flex;flex-direction:row;align-items:baseline;gap:.2rem}h3,p{margin:0;color:var(--miles_primary_light)}
...
`,U=document.createElement("template");U.innerHTML=`
      <style>
        ...
        `;class X extends HTMLElement{constructor(){super();l(this,"timeFormat",i=>new Intl.DateTimeFormat("no",{month:"short",day:"numeric"}).format(i));this.attachShadow({mode:"open"}).appendChild(U.content.cloneNode(!0)),this.titlEl=this.shadowRoot.querySelector("#title"),this.dateEl=this.shadowRoot.querySelector("#date")}static get observedAttributes(){return["episode_title","published_date","link"]}attributeChangedCallback(i,t,o){i==="episode_title"&&(this.titlEl.textContent=o),i==="published_date"&&(this.dateEl.textContent=`${this.timeFormat(new Date(o))} - MilesPodden`)}}const W="miles-podcast-teaser";customElements.get(W)||customElements.define(W,X);const Ie=`:host{display:inline-block;color:red;--arrow-bg-color: #ffffff;--arrow-color: #b72a26}#arrow-nav{display:flex;padding:.8rem;border-radius:50%;background-color:#94e5db}svg{display:inline-block}
...
`,qe=document.createElement("template");qe.innerHTML=`
      <style>
        ...
        `;class He extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.appendChild(qe.content.cloneNode(!0)),this.bannerEl=e.querySelector("#banner"),this.titleEl=e.querySelector("#title"),this.sloganEl=e.querySelector("#slogan")}static get observedAttributes(){return["image","title","slogan","url","variant"]}attributeChangedCallback(e,i,t){e==="image"&&(this.bannerEl.style.backgroundImage=`url(${t})`),e==="title"&&(this.titleEl.textContent=t),e==="slogan"&&(this.sloganEl.textContent=t)}}const $e="miles-banner";customElements.get($e)||customElements.define($e,He),r.MilesArrow=pe,r.MilesArrowNav=J,r.MilesAuthorCard=ve,r.MilesBanner=He,r.MilesBlogCard=se,r.MilesBulb=E,r.MilesBusinessCard=p,r.MilesButtonAnchor=B,r.MilesCloud=f,r.MilesContactCard=Ce,r.MilesCup=S,r.MilesCurve=x,r.MilesFagbloggTeaser=te,r.MilesImageSlider=Y,r.MilesInfo=O,r.MilesInfoBlock=ye,r.MilesLogo=z,r.MilesOfficeBanner=ce,r.MilesOverlapBlock=ae,r.MilesPodcastCard=Le,r.MilesPodcastTeaser=X,r.MilesPodd=q,r.MilesProfileCard=F,r.MilesZoom=L,r.SiteMenu=_e,Object.defineProperty(r,Symbol.toStringTag,{value:"Module"})});
