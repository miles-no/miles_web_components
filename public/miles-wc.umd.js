...
`,d=document.createElement("template");d.innerHTML=`
  ...
  <div class="heart">
    ...
    `;class Fe extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.appendChild(b.content.cloneNode(!0)),this.svg=e.querySelector("svg")}static get observedAttributes(){return["width","height","color"]}attributeChangedCallback(e,i,t){e==="width"&&this.svg.setAttribute("width",t),e==="height"&&this.svg.setAttribute("height",t),e==="color"&&this.svg.setAttribute("fill",t)}}const w="miles-heart";customElements.get(w)||customElements.define(w,Fe);const y=document.createElement("template");y.innerHTML=`
    <style>
      ${r}
    ...
    <div class="cup">
      ...
      `;class S extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.appendChild(_.content.cloneNode(!0)),this.svg=e.querySelector("svg")}static get observedAttributes(){return["width","height","color"]}attributeChangedCallback(e,i,t){e==="width"&&this.svg.setAttribute("width",t),e==="height"&&this.svg.setAttribute("height",t),e==="color"&&this.svg.setAttribute("fill",t)}}const A="miles-cup";customElements.get(A)||customElements.define(A,S);class L extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.innerHTML=`
        <style>
          ${r}
      ...
      `}}const T="miles-zoom";customElements.get(T)||customElements.define(T,L);const H=document.createElement("template");H.innerHTML=`
          <style>
            ${r}
        ...
        `;class q extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.appendChild(H.content.cloneNode(!0)),this.svgEl=e.querySelector("path")}static get observedAttributes(){return["color"]}attributeChangedCallback(e,i,t){e==="color"&&(t&&t.length>0?(this.svgEl.setAttribute("fill",t),this.svgEl.style.transform="scale(1.03)"):(this.svgEl.setAttribute("fill","currentColor"),this.svgEl.style.transform="scale(1)"))}}const $="miles-pod";customElements.get($)||customElements.define($,q);const z=document.createElement("template");z.innerHTML=`
      <style> 
        :host {
          display: flex;
          align-items: center;
...
`;class R extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.appendChild(z.content.cloneNode(!0)),this.svgEl=e.querySelector("path")}static get observedAttributes(){return["color"]}attributeChangedCallback(e,i,t){e==="color"&&(t&&t.length>0?this.svgEl.setAttribute("fill",t):this.svgEl.setAttribute("fill","currentColor"))}}const N="miles-phone";customElements.get(N)||customElements.define(N,R);const B=document.createElement("template");B.innerHTML=`
      <style>
        :host {
          display: flex;
          align-items: center;
...
`;class P extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.appendChild(B.content.cloneNode(!0)),this.svgEl=e.querySelector("path")}static get observedAttributes(){return["color"]}attributeChangedCallback(e,i,t){e==="color"&&(t&&t.length>0?this.svgEl.setAttribute("fill",t):this.svgEl.setAttribute("fill","currentColor"))}}const j="miles-email";customElements.get(j)||customElements.define(j,P);const Oe=`:host{display:inline-block;width:inherit;box-sizing:border-box}
...
`;class Z extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.appendChild(I.content.cloneNode(!0)),this.svg=e.querySelector("svg")}static get observedAttributes(){return["width","height","color"]}attributeChangedCallback(e,i,t){e==="width"&&this.svg.setAttribute("width",t),e==="height"&&this.svg.setAttribute("height",t),e==="color"&&this.svg.setAttribute("fill",t)}}const F="miles-logo";customElements.get(F)||customElements.define(F,Z);const O=n=>[null,"false"].includes(n)===!1,D=document.createElement("template");D.innerHTML=`
        <style>
          ${r}
      ...
      `;class V extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.append(D.content.cloneNode(!0)),this.buttontarget=e.querySelector("#buttonTarget")}static get observedAttributes(){return["href","color","selected"]}attributeChangedCallback(e,i,t){e==="href"&&this.buttontarget.setAttribute("href",t),e==="color"&&this.style.setProperty("--color",t),e==="selected"&&(this.buttontarget.setAttribute("aria-selected",O(t)),this.buttontarget.classList.toggle("selected",O(t)))}connectedCallback(){}}const U="miles-button-anchor";customElements.get(U)||customElements.define(U,V);const De=`:host{display:inline-block;--width: 215px}.profile-card{display:flex;flex-direction:column;width:var(--width);border-radius:5px;overflow:hidden}figure{border-radius:.5rem;height:var(--width);margin:0 0 .5rem;padding:0}img{background-color:#fff;filter:grayscale(1);width:100%;object-fit:cover;aspect-ratio:1 / 1;animation:fadeIn 1s cubic-bezier(.46,.03,.05,.99)}@keyframes fadeIn{0%{opacity:0}to{opacity:1}}.profile-card__info{display:flex;background-color:var(--miles_secondary_four);padding:1.2rem;min-height:140px;flex-direction:column}.jobtitle{flex-grow:1}.jobtitle p{overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:2;line-clamp:2;-webkit-box-orient:vertical}.description{display:flex;flex-direction:row;align-items:baseline;gap:.2rem}h3,p{margin:0;color:var(--miles_primary_light)}
...
`,Q=document.createElement("template");Q.innerHTML=`
      <style>
        ...
        `;class Y extends HTMLElement{constructor(){super();l(this,"timeFormat",i=>new Intl.DateTimeFormat("no",{month:"short",day:"numeric"}).format(i));this.attachShadow({mode:"open"}).appendChild(Q.content.cloneNode(!0)),this.titlEl=this.shadowRoot.querySelector("#title"),this.dateEl=this.shadowRoot.querySelector("#date")}static get observedAttributes(){return["episode_title","published_date","link"]}attributeChangedCallback(i,t,o){i==="episode_title"&&(this.titlEl.textContent=o),i==="published_date"&&(this.dateEl.textContent=`${this.timeFormat(new Date(o))} - MilesPodden`)}}const ee="miles-podcast-teaser";customElements.get(ee)||customElements.define(ee,Y);const Xe=`:host{display:inline-block;color:red;--arrow-bg-color: #ffffff;--arrow-color: #b72a26}#arrow-nav{display:flex;padding:.8rem;border-radius:50%;background-color:#94e5db}svg{display:inline-block}
...
`,se=document.createElement("template");se.innerHTML=`
  ...
  <div id="slide-wrapper">
    ...
    `;class re extends HTMLElement{constructor(){super();l(this,"goRight",()=>{this.startAutoPlay(!1),Math.abs(this.index)!==0&&(this.index++,this.setActiveDot(this.index),this.slides.style.setProperty("--slides-offset",this.index))});l(this,"goLeft",()=>{this.startAutoPlay(!1),Math.abs(this.index)!==this.numberOfSlides-1&&(this.index--,this.setActiveDot(this.index),this.slides.style.setProperty("--slides-offset",this.index))});l(this,"goToSlide",i=>{let t=parseInt(i.target.dataset.slide)*-1;this.numberOfSlides-1===parseInt(i.target.dataset.slide)&&(t=0),this.startAutoPlay(!1),this.slides.style.setProperty("--slides-offset",t),this.setActiveDot(t)});l(this,"startAutoPlay",i=>{i?this.autoPlay=setInterval(()=>{Math.abs(this.index)===this.numberOfSlides-2?this.index=0:this.index--,this.setActiveDot(this.index),this.slides.style.setProperty("--slides-offset",this.index)},5e3):clearInterval(this.autoPlay)});l(this,"setActiveDot",i=>{const t=this.controls.querySelectorAll(".nav-dot");t.forEach(o=>{o.classList.remove("active")}),t.forEach(o=>{parseInt(o.getAttribute("data-slide"))===Math.abs(i)&&o.classList.add("active")})});const i=this.attachShadow({mode:"open"});i.appendChild(se.content.cloneNode(!0)),this.slides=i.querySelector(".slides"),this.controls=i.querySelector(".controls"),this.wrapper=i.querySelector("#slide-wrapper"),this.numberOfSlides=0,this.autoPlay=0,this.index=0,this.logos=["equinor","fjordkraft","tv2","cutters","scaleaq"],this.rightNav=i.querySelector("#rightnav"),this.leftNav=i.querySelector("#leftnav")}connectedCallback(){let i={root:null,rootMargin:"0px",threshold:1};this.observer=new IntersectionObserver(o=>{o.forEach(a=>{a.isIntersecting?this.startAutoPlay(!0):this.startAutoPlay(!1)})},i),this.observer.observe(this.wrapper),this.controls.addEventListener("click",this.goToSlide),this.rightNav.addEventListener("click",this.goLeft),this.leftNav.addEventListener("click",this.goRight);const t=this.shadowRoot.querySelector("slot");if(t){let o=t.assignedElements().length===1?t.assignedElements()[0].children:t.assignedElements();if(typeof o=="object")try{const a=Array.from(o);this.numberOfSlides=o.length,a.forEach((c,Ze)=>{const m=document.createElement("figure"),g=document.createElement("div");if(g.setAttribute("class","overlay"),c.classList.forEach(h=>{this.logos.includes(h)&&g.classList.add(h)}),m.setAttribute("data-slide-image",Ze),m.appendChild(g),c.querySelector("img")){const h=c.querySelector("img");h.setAttribute("draggable",!1),m.appendChild(h),this.slides.appendChild(m);const u=document.createElement("span");u.setAttribute("class","nav-dot"),u.setAttribute("data-slide",Ze),this.controls.appendChild(u)}})}catch{console.log(o)}}this.setActiveDot(this.index)}disconnectedCallback(){this.controls.removeEventListener("click",this.goToSlide),this.observer.unobserve(this.wrapper),this.rightNav.removeEventListener("click",this.goLeft),this.leftNav.removeEventListener("click",this.goRight)}static get observedAttributes(){return["inview","autoplay"]}attributeChangedCallback(i,t,o){}}const ne="miles-image-slider";customElements.get(ne)||customElements.define(ne,re);const Ge=`:host{display:block;color:inherit;--maxWidth: 350px;margin-left:10vw;margin-right:10vw}#header{display:flex;max-width:var(--max-page-width);margin:2rem auto;flex-direction:column}#fagblogg-teaser{box-sizing:border-box;width:100%;max-width:var(--max-page-width);position:relative;min-height:800px;margin:auto}.bg{width:100%;height:50%;position:relative;top:5rem}miles-info{position:relative;width:300px}::slotted(ul){grid-template-columns:repeat(1,100%)!important;display:grid!important;gap:2rem!important}@media (min-width: 768px){::slotted(ul){grid-template-columns:repeat(3,calc(33% - 1rem))!important;display:grid!important;gap:2rem!important}#header{flex-direction:row}.podcast-teaser{height:210px}}@media (max-width: 1024px) and (min-width: 769px){miles-info{left:unset;top:1em}.bg{width:100%;height:35%;position:absolute;top:5rem}}@media (min-width: 1025px){miles-info{left:unset;width:unset}}.left{display:flex;flex-direction:row;align-items:center;gap:1em}.text{display:flex;flex-direction:column}h2{margin-bottom:.2rem;line-height:150%;font-size:var(--miles-h2);color:var(--miles_secondary_four);flex-grow:1}@media (min-width: 769px){h2{margin-bottom:1rem}}p{font-size:.8em;color:var(--miles_primary_dark);margin:auto}svg{height:70px;width:70px}a{color:inherit;right:var(--halve_margin_offset_large);bottom:1em;position:absolute;display:inline-block}a:after{content:"\\2192"}
...
`;class ae extends HTMLElement{constructor(){super();l(this,"goToPost",i=>{window.location.href=escape(i.target.dataset.post)});this.attachShadow({mode:"open"}),this.shadowRoot.append(le.content.cloneNode(!0)),this.mic=this.shadowRoot.querySelector("#mic")}connectedCallback(){const i=this.shadowRoot.querySelector("slot").assignedElements()[0];if(i){const t=i.querySelectorAll("img"),o=Array.from(i.querySelectorAll("a"));Array.from(t).forEach((a,c)=>{a.setAttribute("style","height: auto; cursor: pointer;"),a.setAttribute("data-post",o[c].href),a.addEventListener("click",this.goToPost)})}}disconnectedCallback(){}}const de="miles-fagblogg-teaser";customElements.get(de)||customElements.define(de,ae);const Je=`:host{display:block;color:inherit;width:inherit;height:inherit}#blog-card{display:flex;flex-direction:column;gap:.2rem;width:100%;border-radius:1rem;overflow:hidden}slot[name=title]::slotted(*){font-size:1.5rem;margin:0}a{display:block;width:100%;text-decoration:none;color:inherit}.text{background-color:var(--miles_secondary_four);color:var(--miles_primary_light);padding:2rem;height:110px;display:flex;flex-direction:column}slot[name=title]::slotted(*){overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:2;line-clamp:2;-webkit-box-orient:vertical}slot[name=meta]{display:block;flex-grow:1}#updated{display:none}::slotted(figure){margin:0;aspect-ratio:4/5;overflow:hidden}
...
`,Ce=document.createElement("template");Ce.innerHTML=`
  ...
  <div id="card-wrapper">
    ...
    `;class p extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.append(Ce.content.cloneNode(!0)),this.emailEl=this.shadowRoot.querySelector("#email_el"),this.nameEl=this.shadowRoot.querySelector("#name"),this.phoneEl=this.shadowRoot.querySelector("#phone_el"),this.phoneAnchor=this.shadowRoot.querySelector("#phone"),this.mailAnchor=this.shadowRoot.querySelector("#email"),this.titleEl=this.shadowRoot.querySelector("#title"),this.firgureEl=this.shadowRoot.querySelector("figure"),this.card=this.shadowRoot.querySelector("#card")}static get observedAttributes(){return["email","name","phone","jobtitle","image","variant"]}attributeChangedCallback(e,i,t){if(e==="email"&&(this.mailAnchor.setAttribute("href",`mailto:${t}`),this.mailAnchor.textContent=t),e==="name"&&(this.nameEl.textContent=t),e==="phone"&&(this.phoneAnchor.setAttribute("href",`tel:${t}`),this.shadowRoot.querySelector("#phone").style.display="block",this.phoneAnchor.textContent=t),e==="jobtitle"&&(this.titleEl.textContent=t),e==="image"){const o=document.createElement("img");o.setAttribute("src",t),this.firgureEl.append(o)}e==="variant"&&(t==="wide"&&(this.card.classList.add("wide"),this.style.setProperty("--image-width","320px")),t==="author"&&(this.card.classList.add("author"),this.style.setProperty("--image-width","180px")))}}const Ee="miles-business-card";customElements.get(Ee)||customElements.define(Ee,p);class ke extends p{constructor(){super()}connectedCallback(){super.connectedCallback(),this.setAttribute("variant","author")}attributeChangedCallback(e,i,t){e!=="phone"&&e!=="email"&&super.attributeChangedCallback(e,i,t)}}const Me="miles-author-card";customElements.get(Me)||customElements.define(Me,ke);const tt=`:host{display:block;color:inherit}@media (min-width: 768px){:host{padding-left:calc(8rem - 2em)}}#wrapper{border:4px solid var(--miles_secondary_four);color:var(--miles_secondary_three);border-radius:1em;padding:2em;position:relative;line-height:2.5em}#heading{max-width:100%;background-color:var(--miles_secondary_four);color:var(--miles_primary_light);border-radius:1em;padding:1em 2em;position:relative;z-index:5;left:-4rem;margin-bottom:4rem}h2{margin:0}@media (min-width: 768px){#heading{max-width:60%;left:-8rem;border-radius:2em}#wrapper{border-radius:2em}h2{margin:revert}}p,p::slotted(*){font-size:1rem;line-height:2.5rem;font-weight:400}
`,_e=document.createElement("template");_e.innerHTML=`
  <style>
	${tt}
  ...
  ${r}
    ...
    `;class Se extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.append(_e.content.cloneNode(!0)),this.headingEl=this.shadowRoot.querySelector("h2")}static get observedAttributes(){return["heading"]}attributeChangedCallback(e,i,t){e==="heading"&&(this.headingEl.textContent=t)}connectedCallback(){}disconnectedCallback(){}}const Ae="miles-info-block";customElements.get(Ae)||customElements.define(Ae,Se);const it=`:host{display:inline-block;color:inherit}#wrapper{background-color:var(--miles_secondary_four);display:flex;flex-direction:column}h2{margin:0}a{text-decoration:none;color:inherit}#address,#phone{margin-bottom:1rem}
`,Le=document.createElement("template");Le.innerHTML=`
  <style>
	${it}
...
`,qe=document.createElement("template");qe.innerHTML=`
      ...
      `;class $e extends HTMLElement{constructor(){super();l(this,"openmenu",i=>{console.log("menu toggle"),i.preventDefault(),this.burgerEl.classList.toggle("open-menu"),this.menuContent.classList.toggle("open")});const i=this.attachShadow({mode:"open"});i.append(qe.content.cloneNode(!0)),this.burgerEl=i.querySelector("#nav-icon"),this.menuContent=i.querySelector("#menu-content")}connectedCallback(){this.shadowRoot.querySelector("slot").assignedElements().forEach(i=>{i.tagName==="NAV"&&(i.querySelector('a[href="#"]').remove(),this.menuContent.append(i))}),this.burgerEl.addEventListener("click",this.openmenu)}disconnectedCallback(){this.burgerEl.removeEventListener("click",this.openmenu)}}const ze="miles-site-menu";customElements.get(ze)||customElements.define(ze,$e);const st="",Re=document.createElement("template");Re.innerHTML=`
      <style>
      	${st}
...
`,Pe=document.createElement("template");Pe.innerHTML=`
      <style>
        ...
        `;class je extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.appendChild(Pe.content.cloneNode(!0)),this.bannerEl=e.querySelector("#banner"),this.titleEl=e.querySelector("#title"),this.sloganEl=e.querySelector("#slogan")}static get observedAttributes(){return["image","title","slogan","url","variant"]}attributeChangedCallback(e,i,t){e==="image"&&(this.bannerEl.style.backgroundImage=`url(${t})`),e==="title"&&(this.titleEl.textContent=t),e==="slogan"&&(this.sloganEl.textContent=t)}}const Ie="miles-banner";customElements.get(Ie)||customElements.define(Ie,je),s.MilesArrow=ye,s.MilesArrowNav=ie,s.MilesAuthorCard=ke,s.MilesBanner=je,s.MilesBlogCard=he,s.MilesBulb=k,s.MilesBusinessCard=p,s.MilesButtonAnchor=V,s.MilesCloud=f,s.MilesContactCard=Te,s.MilesCup=S,s.MilesCurve=x,s.MilesEmail=P,s.MilesFagbloggTeaser=ae,s.MilesImageSlider=re,s.MilesInfo=J,s.MilesInfoBlock=Se,s.MilesLogo=Z,s.MilesOfficeBanner=ve,s.MilesOverlapBlock=ge,s.MilesPhone=R,s.MilesPodcastCard=Ne,s.MilesPodcastTeaser=Y,s.MilesPodd=q,s.MilesProfileCard=W,s.MilesZoom=L,s.SiteMenu=$e,Object.defineProperty(s,Symbol.toStringTag,{value:"Module"})});
