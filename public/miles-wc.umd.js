...
`,d=document.createElement("template");d.innerHTML=`
  ...
  <div class="heart">
    ...
    <div class="cup">
      ...
      `}}const T="miles-zoom";customElements.get(T)||customElements.define(T,A);class H extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.innerHTML=`
          <style>
            ${s}
        ...
        `}}const q="miles-pod";customElements.get(q)||customElements.define(q,H);const Ae=`:host{display:inline-block;width:inherit;box-sizing:border-box}
...
`;class z extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.appendChild($.content.cloneNode(!0)),this.svg=e.querySelector("svg")}static get observedAttributes(){return["width","height","color"]}attributeChangedCallback(e,i,t){e==="width"&&this.svg.setAttribute("width",t),e==="height"&&this.svg.setAttribute("height",t),e==="color"&&this.svg.setAttribute("fill",t)}}const N="miles-logo";customElements.get(N)||customElements.define(N,z);const R=document.createElement("template");R.innerHTML=`
      ...
      `;class B extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.append(R.content.cloneNode(!0)),this.buttontarget=e.querySelector("#buttonTarget")}static get observedAttributes(){return["href","color"]}attributeChangedCallback(e,i,t){e==="href"&&this.buttontarget.setAttribute("href",t),e==="color"&&this.style.setProperty("--color",t)}connectedCallback(){}}const P="miles-button-anchor";customElements.get(P)||customElements.define(P,B);const Te=`:host{display:inline-block;--width: 215px}.profile-card{display:flex;flex-direction:column;width:var(--width);border-radius:5px;overflow:hidden}figure{border-radius:.5rem;height:var(--width);margin:0 0 .5rem;padding:0}img{background-color:#fff;filter:grayscale(1);width:100%;object-fit:cover;aspect-ratio:1 / 1}.profile-card__info{display:flex;background-color:var(--miles_secondary_four);padding:1.2rem;min-height:140px;flex-direction:column}.jobtitle{flex-grow:1}.jobtitle p{overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:2;line-clamp:2;-webkit-box-orient:vertical}.description{display:flex;flex-direction:row;align-items:baseline;gap:.2rem}h3,p{margin:0;color:var(--miles_primary_light)}
...
`,D=document.createElement("template");D.innerHTML=`
      <style>
        ...
        `;class U extends HTMLElement{constructor(){super();l(this,"timeFormat",i=>new Intl.DateTimeFormat("no",{month:"short",day:"numeric"}).format(i));this.attachShadow({mode:"open"}).appendChild(D.content.cloneNode(!0)),this.titlEl=this.shadowRoot.querySelector("#title"),this.dateEl=this.shadowRoot.querySelector("#date")}static get observedAttributes(){return["episode_title","published_date","link"]}attributeChangedCallback(i,t,o){i==="episode_title"&&(this.titlEl.textContent=o),i==="published_date"&&(this.dateEl.textContent=`${this.timeFormat(new Date(o))} - MilesPodden`)}}const V="miles-podcast-teaser";customElements.get(V)||customElements.define(V,U);const $e=`:host{display:inline-block;color:red;--arrow-bg-color: #ffffff;--arrow-color: #b72a26}#arrow-nav{display:flex;padding:.8rem;border-radius:50%;background-color:#94e5db}svg{display:inline-block}
...
`,Ee=document.createElement("template");Ee.innerHTML=`
      ...
      `;class Me extends HTMLElement{constructor(){super();l(this,"openmenu",i=>{console.log("menu toggle"),i.preventDefault(),this.burgerEl.classList.toggle("open-menu"),this.menuContent.classList.toggle("open")});const i=this.attachShadow({mode:"open"});i.append(Ee.content.cloneNode(!0)),this.burgerEl=i.querySelector("#nav-icon"),this.menuContent=i.querySelector("#menu-content")}connectedCallback(){this.shadowRoot.querySelector("slot").assignedElements().forEach(i=>{i.tagName==="NAV"&&(i.querySelector('a[href="#"]').remove(),this.menuContent.append(i))}),this.burgerEl.addEventListener("click",this.openmenu)}disconnectedCallback(){this.burgerEl.removeEventListener("click",this.openmenu)}}const _e="miles-site-menu";customElements.get(_e)||customElements.define(_e,Me),r.MilesArrow=me,r.MilesArrowNav=X,r.MilesAuthorCard=fe,r.MilesBlogCard=oe,r.MilesBulb=E,r.MilesBusinessCard=p,r.MilesButtonAnchor=B,r.MilesCloud=f,r.MilesContactCard=ke,r.MilesCup=S,r.MilesCurve=x,r.MilesFagbloggTeaser=ee,r.MilesImageSlider=K,r.MilesInfo=F,r.MilesInfoBlock=we,r.MilesLogo=z,r.MilesOfficeBanner=de,r.MilesOverlapBlock=ne,r.MilesPodcastTeaser=U,r.MilesPodd=H,r.MilesProfileCard=I,r.MilesZoom=A,r.SiteMenu=Me,Object.defineProperty(r,Symbol.toStringTag,{value:"Module"})});
