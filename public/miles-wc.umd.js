...
`,d=document.createElement("template");d.innerHTML=`
  ...
  <div class="heart">
    ...
    `;class Se extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.appendChild(b.content.cloneNode(!0)),this.svg=e.querySelector("svg")}static get observedAttributes(){return["width","height","color"]}attributeChangedCallback(e,i,t){e==="width"&&this.svg.setAttribute("width",t),e==="height"&&this.svg.setAttribute("height",t),e==="color"&&this.svg.setAttribute("fill",t)}}const w="miles-heart";customElements.get(w)||customElements.define(w,Se);const y=document.createElement("template");y.innerHTML=`
    <style>
      ${o}
    ...
    <div class="cup">
      ...
      `}}const T="miles-zoom";customElements.get(T)||customElements.define(T,A);class H extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.innerHTML=`
        ...
        `}}const q="miles-pod";customElements.get(q)||customElements.define(q,H);const Le=`:host{display:inline-block;width:inherit;box-sizing:border-box}
...
`;class D extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.innerHTML=`
      <style>
        ...
        `}}const U="miles-podcast-teaser";customElements.get(U)||customElements.define(U,D);const qe=`:host{display:inline-block;color:red;--arrow-bg-color: #ffffff;--arrow-color: #b72a26}#arrow-nav{display:flex;padding:.8rem;border-radius:50%;background-color:#94e5db}svg{display:inline-block}
...
`,Ce=document.createElement("template");Ce.innerHTML=`
      ...
      `;class Ee extends HTMLElement{constructor(){super();l(this,"openmenu",i=>{console.log("menu toggle"),i.preventDefault(),this.burgerEl.classList.toggle("open-menu"),this.menuContent.classList.toggle("open")});const i=this.attachShadow({mode:"open"});i.append(Ce.content.cloneNode(!0)),this.burgerEl=i.querySelector("#nav-icon"),this.menuContent=i.querySelector("#menu-content")}connectedCallback(){this.shadowRoot.querySelector("slot").assignedElements().forEach(i=>{i.tagName==="NAV"&&(i.querySelector('a[href="#"]').remove(),this.menuContent.append(i))}),this.burgerEl.addEventListener("click",this.openmenu)}disconnectedCallback(){this.burgerEl.removeEventListener("click",this.openmenu)}}const Me="miles-site-menu";customElements.get(Me)||customElements.define(Me,Ee),s.MilesArrow=he,s.MilesArrowNav=W,s.MilesAuthorCard=ue,s.MilesBlogCard=ie,s.MilesBulb=E,s.MilesBusinessCard=p,s.MilesButtonAnchor=B,s.MilesCloud=f,s.MilesContactCard=xe,s.MilesCup=S,s.MilesCurve=x,s.MilesFagbloggTeaser=Y,s.MilesImageSlider=J,s.MilesInfo=O,s.MilesInfoBlock=be,s.MilesLogo=z,s.MilesOfficeBanner=le,s.MilesOverlapBlock=re,s.MilesPodcastTeaser=D,s.MilesPodd=H,s.MilesProfileCard=I,s.MilesZoom=A,s.SiteMenu=Ee,Object.defineProperty(s,Symbol.toStringTag,{value:"Module"})});
