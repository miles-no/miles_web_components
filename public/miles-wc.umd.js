...
`;class N extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.appendChild(z.content.cloneNode(!0)),this.svgEl=e.querySelector("path")}static get observedAttributes(){return["color"]}attributeChangedCallback(e,i,t){e==="color"&&(t&&t.length>0?this.svgEl.setAttribute("fill",t):this.svgEl.setAttribute("fill","currentColor"))}}const R="miles-phone";customElements.get(R)||customElements.define(R,N);const P=document.createElement("template");P.innerHTML=`
      <style>
        :host {
          display: flex;
          align-items: center;
...
`;class B extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.appendChild(P.content.cloneNode(!0)),this.svgEl=e.querySelector("path")}static get observedAttributes(){return["color"]}attributeChangedCallback(e,i,t){e==="color"&&(t&&t.length>0?this.svgEl.setAttribute("fill",t):this.svgEl.setAttribute("fill","currentColor"))}}const j="miles-email";customElements.get(j)||customElements.define(j,B);const Oe=`:host{display:inline-block;width:inherit;box-sizing:border-box}
...
`,Be=document.createElement("template");Be.innerHTML=`
      <style>
        ...
        `;class je extends HTMLElement{constructor(){super();const e=this.attachShadow({mode:"open"});e.appendChild(Be.content.cloneNode(!0)),this.bannerEl=e.querySelector("#banner"),this.titleEl=e.querySelector("#title"),this.sloganEl=e.querySelector("#slogan")}static get observedAttributes(){return["image","title","slogan","url","variant"]}attributeChangedCallback(e,i,t){e==="image"&&(this.bannerEl.style.backgroundImage=`url(${t})`),e==="title"&&(this.titleEl.textContent=t),e==="slogan"&&(this.sloganEl.textContent=t)}}const Ie="miles-banner";customElements.get(Ie)||customElements.define(Ie,je),s.MilesArrow=ye,s.MilesArrowNav=ie,s.MilesAuthorCard=Ee,s.MilesBanner=je,s.MilesBlogCard=he,s.MilesBulb=E,s.MilesBusinessCard=p,s.MilesButtonAnchor=V,s.MilesCloud=f,s.MilesContactCard=Te,s.MilesCup=S,s.MilesCurve=x,s.MilesEmail=B,s.MilesFagbloggTeaser=ae,s.MilesImageSlider=re,s.MilesInfo=J,s.MilesInfoBlock=Se,s.MilesLogo=Z,s.MilesOfficeBanner=ve,s.MilesOverlapBlock=ge,s.MilesPhone=N,s.MilesPodcastCard=Re,s.MilesPodcastTeaser=Y,s.MilesPodd=q,s.MilesProfileCard=W,s.MilesZoom=L,s.SiteMenu=$e,Object.defineProperty(s,Symbol.toStringTag,{value:"Module"})});
